...
function Banner({netflixOriginals}:Props) {
  ...
  return (
    <div className="flex flex-col space-y-2 py-16 md:space-y-4 lg:h-[65vh] lg:justify-end lg:pb-12">
      ...
      <p className="max-w-xs text-shadow-md text-sm md:max-w-lg md:text-lg lg:max-w-2xl lg:text-2xl">{movie?.overview}</p>
      <div className="flex space-x-3">
        ...
        >
          ...
          <FaPlay className="h-4 w-4 text-black md:h-7 :w-7 "/> Play
        </button>
        <button
          onClick={()=> {
            setCurrentMovie(movie)
