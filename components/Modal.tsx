...
function Modal() {
  ...
  useEffect(() => {
    ...
    async function fetchMovie(){
      ...
      }&language=en-US&append_to_response=videos`).then((response) => response.json()).catch((error) =>console.log(error.message));

      console.log(data);
      if(data?.videos) {
        const index = data.videos.results.findIndex((element: Element) => element.type === "Trailer")

        // * necesitamos el key porque es lo que nos permite renderizar un video en este caso de youtube
